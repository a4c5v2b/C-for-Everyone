/* C for Everyone: Structured Programming
	Week 2: Using a stack to reverse a string */
...
char pop(stack* stk)
{
	return(stk->s[stk->top--]);  // top-- after the current top element is popped
}
...
{
	return(stk->top == EMPTY);  // EMPTY = -1
}

int is_full(const stack* stk)
{
	return(stk->top == FULL);  // FULL = MAX_LEN - 1
}
